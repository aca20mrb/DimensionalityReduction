...
for method in methods.keys():
    method_data = silhouette_data[silhouette_data['Method'] == method]
    ax1.bar(method, method_data['Silhouette Score'].mean(), yerr=method_data['Silhouette Score'].std(), capsize=5)
ax1.set_title('Comparison of Dimensionality Reduction Techniques with K-Means Clustering - Silhouette Score', color='black')
...
plt.savefig('ophthomology_silhouette_plot.png')
plt.show()
...
ax2 = plt.subplot()
for method in methods.keys():
    method_data = davies_bouldin_data[davies_bouldin_data['Method'] == method]
    ax2.bar(method, method_data['Davies-Bouldin Score'].mean(), yerr=method_data['Davies-Bouldin Score'].std(), capsize=5)
ax2.set_title('Comparison of Dimensionality Reduction Techniques with K-Means Clustering - Davies-Bouldin Score', color='black')
...
plt.ylim(0.4, 0.7)
plt.tight_layout(pad=2)  # Adjust the padding to ensure nothing gets cut off
plt.savefig('ophthomology_davies_bouldin_plot.png')
plt.show()

# Scatter plots for cluster maps
for method, (X_reduced, clusters) in cluster_maps.items():
    ...
    plt.title(f'Ophthalmology Cluster map for {method} with K-Means')
    plt.xlabel('Component A')
    plt.ylabel('Component B')
    plt.legend(*scatter.legend_elements(), title='Cluster')
    plt.xticks([])
    plt.yticks([])
    plt.savefig(f'opticalplots/cluster_map_{method}.png')
    plt.show()

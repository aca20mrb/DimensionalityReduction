...
#print('Target Wines: ',y)
#print('Target Wines Type: ',type(y))
...
n_iterations = 10  # Number of iterations to average results
...
results = {method: {'Silhouette': [], 'Davies-Bouldin': [], 'Rand-Index': [], 'Time': []} for method in methods}
cluster_maps = {}
...
for method, scores in results.items():
    ...
    print(f"  Rand Index: {np.mean(scores['Rand-Index'])}")
...
rand_index_data = pd.DataFrame({method: scores['Rand-Index'] for method, scores in results.items()})
...
rand_index_data = rand_index_data.melt(var_name='Method', value_name='Rand Index')
...
plt.savefig('wine_silhouette_plot.png')
plt.show()
...
plt.yticks(color='black')
plt.tight_layout(pad=2)  # Adjust the padding to ensure nothing gets cut off
plt.savefig('wine_davies_bouldin_plot.png')
plt.show()

# Plot Rand Index Scores
plt.figure(figsize=(14, 10))  # Adjust figure size to give more room
ax3 = sns.barplot(x='Method', y='Rand Index', data=rand_index_data, capsize=.1, palette=palette, ci='sd')
ax3.set_title('Comparison of Dimensionality Reduction Techniques with K-Means Clustering - Rand Index', color='black')
ax3.set_ylabel('Average Rand Index', color='black')
ax3.set_xlabel('Methods', color='black')
plt.xticks(rotation=45, color='black')
plt.yticks(color='black')
plt.tight_layout(pad=2)  # Adjust the padding to ensure nothing gets cut off
plt.savefig('wine_rand_index_plot.png')
plt.show()

# Plot Processing Time
plt.figure(figsize=(14, 10))  # Adjust figure size to give more room
ax4 = sns.barplot(x='Method', y='Processing Time', data=time_data, capsize=.1, palette=palette, ci='sd')
ax4.set_title('Comparison of Dimensionality Reduction Techniques - Processing Time', color='black')
ax4.set_ylabel('Processing Time (milliseconds)', color='black')
ax4.set_xlabel('Methods', color='black')
plt.xticks(rotation=45, color='black')
plt.yticks(color='black')
plt.tight_layout(pad=2)  # Adjust the padding to ensure nothing gets cut off
plt.savefig('wine_processing_time_plot.png')
plt.show()
